...
const SubscriptionPage = () => {
  ...
  /* 🚀 تم تصحيح اسم الجدول إلى "subscriptions" بدلاً من provider_subscriptions */
  /* 🚀 وتم استبدال maybeSingle لجلب المصفوفة بالكامل لتجنب أخطاء التكرار في قاعدة البيانات */
  const fetchSubscription = useCallback(async (mounted = true) => {
    ...
    try {
      ...
      if (!error && mounted && data && data.length > 0) {
        // نبحث عن أي اشتراك فعال، وإلا نأخذ أحدث واحد
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      // 🚀 توجيه المزود فوراً إلى لوحة التحكم مع علامة النجاح لفك الحظر تلقائياً
      setTimeout(() => {
